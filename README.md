...
- **终名下永远不出现半截文件**。长度对得上**且**每个字节都有窗口认领，才把 `.part` 改成真名；
  半成品只可能叫 `.part`。
- **留下的 part 下次接着用**。part 旁边会写一张 `<文件>.part.json` 的**条子**：链接、长度、校验值（有的话）、
  以及**已经落盘的区间**。进程被杀死、链接断了、Ctrl-C 停了，条子和 part 都留着；下次下**同一个链接**
  就按条子说的位置接着下，不再从零。条子对不上（链接 / 长度 / sha256 有一个变了）就把条子和它指名的 part
  一起丢掉——绝不拿一份可能已经变了内容的半成品接着写。区间只在写它的句柄关闭之后才记，所以条子不会声称
  盘上没有的字节。
- **一个目的地同时只有一个进程在写**。`<文件>.part.lock` 是一把排他锁（进程一死内核就放掉，不需要清理）：
  拿不到锁的进程不去碰盘上任何东西，所以两份下载不会写进同一个 part。
...
按钮就是状态机：开始下载 → 暂停 → 继续；旁边的取消 = 删掉 part（暂停留着，能续；**关窗口也算暂停**）。
下次填同一个链接点开始，会从盘上那份继续，状态行会写「接着上次下（已有 1.2 GiB）」。
...
| `--fresh` | 不要接着上次下：删掉留下的 part，从头开始（默认会接着下） |
| `--json` | 每行一个 JSON 事件：`probe` / `resumed` / `progress` / `done` / `stopped` / `failed` |
| `-q/--quiet` | 不打印进度，只打印结论 |

退出码就是结论：`0` 落地、`1` 失败、`2` 校验不符、`130` 被中断。Ctrl-C 是**停下来但保留进度**：
part 和它的条子都留着（界面上那个**取消**按钮才是删掉的那个），下次下同一个链接从条子说的位置接着下。
...
接着上次下的时候，`probe` 之后会多一行 `resumed`（只在真有得接着下时才出现）：

```json
{"event": "resumed", "done": 1288490188, "total": 17546610688}
```
...
43 例测试里的假服务器能提供四种形状：正常 `206`、**忽略 Range 回 200 全量**、**身体写一半就断**、
一律 `416`。据此钉住的都是合同级的性质：并集覆盖率、续传的起点不是 0、`416` 只重探一次、
停滞会让连接数下来（且不再爬回去）、健康链路会让它上去、暂停留 part 而取消删 part、
被 kill 的进程留下的 part 会被下一跑接管（身份对不上就不接管，另一个活着的进程拿着锁时一个字节都不碰）。
命令行那 15 例钉的是它自己的承诺：退出码就是结论、`--json` 每一行都能被 `json.loads`、
`--sha256` 真的核对（不符退出码 2 且不删文件）、信号处理器用完放回去、`--fresh` 真的从头下。

## 实机验证（2026-09-19 / 09-21）
...
- 自适应本身目前由单测（停滞→减流、健康→涨流）与界面验收覆盖；**这条链路上干净的 A/B 数字还欠着**，
  等链路好的时候补。
- **命令行**（2026-09-21，手机热点链路）：644 MB 的 hf-mirror 文件，Flower CLI 从 2 条流涨到 **4 条**后不再涨，
  一路 500–730 KB/s；同一条链路上手写的 16 连接多源下载器是 435 KB/s——天花板在链路（~0.6 MB/s），
  4 条流就到顶。落地 sha256 与发布方公布的 LFS 值一致，无 part 残留。
- 命令行 Ctrl-C 实测（2026-09-21 旧行为 / **2026-09-23 改了**）：现在打印「已中断，part 保留着」→ 退出码
  **130** → 目标目录里留着 `.part` 和它的 `.part.json` 条子，**再跑同一条链接就从条子说的位置接着下**；
  要删掉的是界面上的「取消」和命令行的 `--fresh`。改之前的表现是「part 删掉、只留 130」。
- 反面实例（同一天，值得知道）：一次运行在坏链路上吃满停滞后，自适应把连接数降下来且**不再涨回去**；
  随后换到健康的链路，那一次运行仍然只有 2 条流（130 KB/s）。**重开一次**同一条链接才回到 4 条流。
  这是设计使然（停滞过的不再重试），代价是「链路变好了要重开」。
- 换源比加连接值钱（同一天、同一份 DiT）：单连接 hf-mirror **77 KB/s** vs ModelScope **338 KB/s**；
  ModelScope 4/8/16 连接 = 885 KB/s / 1.76 MB/s / **2.4 MB/s**。
